...
#elif TAG_ID == 2
  #define SRC_ADDR        0x1122334455667799
  #define DEST_ADDR       0x8877665544332211 /* this is the address of the responder */
  #define DEST_PAN_ID     0x1234             /* this is the PAN ID used in this example */
#endif

uint8_t SRC_ADDR_ARR[8]  = { U64_TO_BYTES(DEST_ADDR) };
...
#elif TAG_ID == 2
static uint8_t rx_poll_msg[] = {'T','e','s','t',' ','T','e','s','t','T','a','g'};
static uint8_t tx_resp_msg[] = {0,0,0,0,0,0,0,0,'T','e','s','t',' ','T','a','g'};
...
void setup() {
  ...
  Serial.println();
  Serial.print("DEST PAN ID: ");
  Serial.print(DEST_PAN_ID, HEX);
  Serial.println();
  ...
  /* Enabling LEDs here for debug so that for each TX the D1 LED will flash on DW3000 red eval-shield boards.
    ...
    // Enable frame filtering: accept data and ACK frames
    // dwt_configureframefilter(DWT_FF_ENABLE_802_15_4, DWT_FF_DATA_EN | DWT_FF_ACK_EN);
    // dwt_configureframefilter(DWT_FF_DISABLE, 0);

    // Filter by PAN ID (match DEST_PAN_ID from anchor)
    dwt_setpanid(DEST_PAN_ID);
...
void loop() {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            Serial.println("Succesfully decrypted");
            ...
            if (memcmp(rx_buffer, rx_poll_msg, aes_job_rx.payload_len) == 0)
            {
                ...
                {   Serial.println("Frame sent");
                  ...
                  Serial.println("response frame failed to transmit back.");
                }
            }
        }
        else
